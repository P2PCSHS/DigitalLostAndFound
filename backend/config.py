...
class DevelopmentConfig(Config):
    ...
    SQLALCHEMY_DATABASE_URI = os.environ.get("DATABASE_URL") or f"sqlite:///{DATA_DIR / 'dev.db'}"
...
class ProductionConfig(Config):
    ...
    def validate(cls):
        missing = sorted(
            env_name for attr, env_name in cls._REQUIRED.items() if not getattr(cls, attr)
        )
        if missing:
            raise RuntimeError(
